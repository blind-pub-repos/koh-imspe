...

{
  if(any(Sys.info()[4] %in% c("Scotts-MacBook-Pro-2.local","Scotts-MacBook-Pro.local", "Scotts-MBP"))){
    ...
    setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
    file.path <- "~/Documents/PhD Work/gramacy-lab/kohdesign/exp/1d-sinusoid/"
  }else{
...
IMSPE.search <- function (model, replicate = FALSE, Xcand = NULL, control = list(tol_dist = 1e-06, 
  ...
  previous.file <- "03-17-2022-sinusoid-1000reps.RData"
  previous.file.short <- "03-17-2022-sinusoid-1000reps"
