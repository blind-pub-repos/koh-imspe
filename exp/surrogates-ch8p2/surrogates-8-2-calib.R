### Calibration Example Number 2
### Chapter 8, Problem 2 In surrogates

Comp_Model <- function(X,U){
  
  
  numerator <- 1000*U[,1]*X[,1]^3 +1900*X[,1]^2+2092*X[,1] + 60
  denomenator <- 100*U[,2]*X[,1]^3 + 500*X[,1]^2 + 4*X[,1] + 20
  
  y <- (1-exp(-1/(2*X[,2])))*(numerator/denomenator)
...
bias <- function(X){
  numerator <- 10*X[,1]^2 + 4*X[,2]^2
  denomenator <- 50*X[,1]*X[,2] + 10
  return(numerator/denomenator)
}

Field <- function(X, sd = 0.25){
  y <- Comp_Model(X,U = matrix(c(0.2,0.1), nrow = 1, ncol=  2))
  y <- y + bias(X) + rnorm(nrow(X),0,sd = sd)
  return(y)
}
...
GP_Fits <- function(Xm = NULL, yM = NULL, Xf = NULL, yF = NULL, eps = sqrt(.Machine$double.eps), starts = 10, uhat = NULL, previous.params = NULL, sep = NULL){
  ...
  if(sep == TRUE){
    ...
    if(is.null(previous.params)){ 
      d <- darg(d = list(mle= TRUE, ab = c(3/2,5/4), max = 20), X = Xm)
      formals(bhat.fit)$da <- darg(d=list(mle=TRUE, ab = c(3/2,5/2), max = 20), X=Xf)
      formals(bhat.fit)$ga <- garg(g=list(mle=TRUE, ab = c(3/2, 0.05)), y=yF)
    }else{
      d <- darg(d = list(mle= TRUE, ab = c(3/2,5/4), max = 20), X = Xm)
      d$start <- previous.params$theta_m
      temp <- darg(d=list(mle=TRUE, ab = c(3/2,5/2), max = 20), X=Xf)
      temp$start <- previous.params$theta_b
      formals(bhat.fit)$da  <- temp
      formals(bhat.fit)$ga <- garg(g=list(mle=TRUE, ab = c(3/2, 0.05), start = previous.params$gb), y=yF)
    }
...
uhat.calib <- function(u = NULL, Xf = NULL, yF = NULL, yMhat = NULL, Xfu = NULL, theta_b = NULL , gb = NULL, eps = NULL){
  
  # objective function for estimating uhat given a full set of hyperparams
  ...
  bhat <- newGPsep(Xf, resid, d = theta_b, g = gb, dK = FALSE)
  
  cmle <- -llikGPsep(bhat, dab = c(0,0), gab = c(0,0))
...
uhat.est <- function(Xm = NULL, Xf = NULL, yM = NULL, yF = NULL, starts = 50, eps = sqrt(.Machine$double.eps),
                     Vb = 839.601060559771, Vm = 18.6286883891487, theta_b1 = 1.40053680891583, 
                       theta_b2 = 1.47278070772745, theta_m1 = 0.0932946036561526, theta_m2 = 0.387151715458473, 
                       theta_m3 = 6.17505379625791, theta_m4 = 5.74438473211422, gb = 8.01598506407026e-05){
  
  ### Wrapper Function for extimating uhat, given a set of "known" hyperparameters
  ### Defaults are median values calculated over all tests found within 02-21-2022-surrogates-250reps.RData
  
  theta_b <- c(theta_b1, theta_b2)
  theta_m <- c(theta_m1, theta_m2, theta_m3, theta_m4)
  
  
  yMhat <- newGPsep(Xm, yM, d= theta_m, g = eps, dK=FALSE) #Fix nugget b/c deterministic model
  ...
  #Optimize u
  obj <- function(x = NULL, Xf = NULL, yF = NULL, yMhat = NULL, Xfu = NULL, theta_b = NULL, gb = NULL, eps = NULL) {
    if(any(x < 0) || any(x > 1)) return(Inf)
    
    uhat.calib(u = x, Xf = Xf,yF = yF, yMhat = yMhat, Xfu = Xfu, theta_b = theta_b, gb = gb, eps = eps) - lprior(x)
  }
  ...
  Xfu <- cbind(Xf, matrix(NA,nrow = nrow(Xf), ncol=ncol(Xm)-ncol(Xf)))
    ...
    for(i in 1:starts){
      soln <-  optim(proposed.uhats[i,],obj, method = "Nelder-Mead", Xf=Xf, yF = yF, yMhat = yMhat, Xfu = Xfu, theta_b = theta_b, gb = gb ,eps = eps)
      uhats[i,] <- c(soln$value, soln$par)
    }
